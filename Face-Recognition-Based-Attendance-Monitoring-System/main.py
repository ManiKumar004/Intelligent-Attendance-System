...
def contact():
    mess._show(title='Contact us', message="Please contact us on : 'kondamanikumar04@gmail.com' ")
...
email_domains = ["gmail.com", "yahoo.com", "iare.ac.in"]
...
def send_email():
    ...
    from_email = "kondamanikumar04@gmail.com"  # Update with your email
    password = "fclp dawh nswx cfrh"  # Use App Password if 2FA is enabled
    ...
    if not os.path.isfile(filename):
        mess._show(title='Error', message=f'File not found: {filename}')
        return
    ...
    try:
        ...
        mess._show(title='Error', message=f'Failed to read the attachment: {str(e)}')
        return

    # Send the email
    try:
        server = smtplib.SMTP('smtp.gmail.com', 587)
        server.starttls()  # Ensure that TLS is started
        server.login(from_email, password)  # Use App Password if 2FA is enabled
        text = msg.as_string()
        ...
        mess._show(title='Error', message='Failed to connect to the SMTP server.')
    except smtplib.SMTPException as e:
        mess._show(title='Error', message=f'SMTP error occurred: {str(e)}')
    except Exception as e:
        mess._show(title='Error', message=f'An unexpected error occurred: {str(e)}')
'''
...
def send_email():
    ...
    from_email = "kondamanikumar04@gmail.com"  # Update with your email
    password = "jfclp dawh nswx cfrh"  # Update with your email password
